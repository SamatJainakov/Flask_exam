    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            try:
                ...
                flash('Такой пользователь уже сущществует', 'danger')
                return render_template('standard_form.html', form=form)
            else:
                flash('Вы успешно создали должность!', 'success')
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            try:
                ...
                flash('Вы успешно создали сотрудника!', 'success')
                return redirect(url_for('index'))
        else:
            print(form.errors)
    return render_template('standard_form.html', form=form)
...
def employee_update(id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash('Вы успешно обновили сотрудника!', 'success')
            return redirect(url_for('index'))
    else:
        print(form.errors)
    return render_template('standard_form.html', form=form)
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            try:
                ...
                flash('Такой пользователь не сущществует', 'danger')
